...
@export var throw_starting_height : float = 32
...
func player_interact() -> void:
	if GlobalPlayerManager.interact_handled == true:
		return
	if picked_up == false:
		GlobalPlayerManager.interact_handled = true
		disable_collisons(throwable)
		if throwable.get_parent():
			throwable.get_parent().remove_child(throwable)
		GlobalPlayerManager.player.held_item.add_child(throwable)
...
func disable_collisons(_node : Node) -> void:
...
func throw() -> void:
	throwable.get_parent().remove_child(throwable)
	GlobalPlayerManager.player.get_parent().call_deferred("add_child", throwable)
	throwable.position = GlobalPlayerManager.player.position
