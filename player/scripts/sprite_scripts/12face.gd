...
func update_sprite():
	...
	if current_sprite == "none":
		...
		face_sprite.material = null
	else:
		face_sprite.texture = Global.face_collection[current_sprite]
		face_sprite.material = get_palette_shader(palettes["3color"]["base"], palettes["3color"]["palettes"][singlethreecolours])
	
	Global.selected_face = current_sprite
	Global.selected_face_color = face_sprite.material
...
func _on_collection_button_pressed() -> void:
	current_face_index = (current_face_index + 1) % face_keys.size()
	update_sprite()
...
func _on_random_button_pressed() -> void:
	current_face_index = rng_sprite.randi_range(1, Global.face_collection.size() - 1) % face_keys.size()
