...
func update_sprite():
	...
	if current_sprite == "none":
		...
		hair_sprite.material = get_palette_shader(palettes["haircolor"]["base"], palettes["haircolor"]["palettes"][haircolours])
	
	Global.selected_hair = current_sprite
	Global.selected_hair_color = hair_sprite.material
...
func _on_color_button_pressed() -> void:
	haircolours = (haircolours + 1) % palettes["haircolor"]["palettes"].size()
	print("Hair Colour:")
	print(haircolours)
	update_sprite()

func _on_random_button_pressed() -> void:
	current_hair_index = rng_sprite.randi_range(1, Global.hair_collection.size() - 1) % hair_keys.size()
	haircolours = rng_colors.randi_range(1, palettes["haircolor"]["palettes"].size() - 1) % palettes["haircolor"]["palettes"].size()
	update_sprite()
