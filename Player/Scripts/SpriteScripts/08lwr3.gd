...
# Node ref
@onready var skirt_sprite = $Sprite2D
...
func set_sprite_keys():
	skirt_keys = Global.skirt_collection.keys()

# Updates texture & modulate
func update_sprite():
	var current_sprite = skirt_keys[current_skirt_index]	
	if current_sprite == "none":
		skirt_sprite.texture = null
		skirt_sprite.material = null
	else:
		skirt_sprite.texture = Global.skirt_collection[current_sprite]
		skirt_sprite.material = get_palette_shader(palettes["3color"]["base"], palettes["3color"]["palettes"][singlethreecolours])
	
	Global.selected_skirt = current_sprite
	Global.selected_skirt_color = skirt_sprite.material
...
func _on_collection_button_pressed() -> void:
	current_skirt_index = (current_skirt_index + 1) % skirt_keys.size()
	update_sprite()

func _on_color_button_pressed() -> void:
	singlethreecolours = (singlethreecolours + 1) % palettes["3color"]["palettes"].size()
	print("Clothing Colour:")
	print(singlethreecolours)
	update_sprite()

func _on_random_button_pressed() -> void:
	current_skirt_index = rng_sprite.randi_range(1, Global.skirt_collection.size() - 1) % skirt_keys.size()
