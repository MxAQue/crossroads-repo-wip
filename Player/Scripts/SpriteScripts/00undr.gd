...
# Node ref
@onready var underneath_sprite = $Sprite2D
...
# Keys
var underneath_keys = []
var current_underneath_index = 0
...
func set_sprite_keys():
	underneath_keys = Global.underneath_collection.keys()

# Updates texture & modulate
func update_sprite():
	var current_sprite = underneath_keys[current_underneath_index]	
	if current_sprite == "none":
		underneath_sprite.texture = null
		underneath_sprite.material = null
	else:
		underneath_sprite.texture = Global.underneath_collection[current_sprite]
		underneath_sprite.material = get_palette_shader(palettes["3color"]["base"], palettes["3color"]["palettes"][singlethreecolours])
	
	Global.selected_underneath = current_sprite
	Global.selected_underneath_color = underneath_sprite.material
...
func _on_collection_button_pressed() -> void:
	current_underneath_index = (current_underneath_index + 1) % underneath_keys.size()
	update_sprite()
...
func _on_random_button_pressed() -> void:
	current_underneath_index = rng_sprite.randi_range(1, Global.underneath_collection.size() - 1) % underneath_keys.size()
