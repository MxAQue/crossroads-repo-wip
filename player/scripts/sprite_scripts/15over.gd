...
# Node ref
@onready var over_sprite = $Sprite2D
...
func set_sprite_keys():
	over_keys = Global.over_collection.keys()

# Updates texture & modulate
func update_sprite():
	var current_sprite = over_keys[current_over_index]	
	if current_sprite == "none":
		over_sprite.texture = null
		over_sprite.material = null
	else:
		over_sprite.texture = Global.over_collection[current_sprite]
		over_sprite.material = get_palette_shader(palettes["3color"]["base"], palettes["3color"]["palettes"][singlethreecolours])
	
	Global.selected_over = current_sprite
	Global.selected_over_color = over_sprite.material
...
func _on_collection_button_pressed() -> void:
	current_over_index = (current_over_index + 1) % over_keys.size()
	update_sprite()
...
func _on_random_button_pressed() -> void:
	current_over_index = rng_sprite.randi_range(1, Global.over_collection.size() - 1) % over_keys.size()
