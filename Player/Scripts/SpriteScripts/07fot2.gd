...
# Node ref
@onready var boots_sprite = $Sprite2D
...
func set_sprite_keys():
	boots_keys = Global.boots_collection.keys()

# Updates texture & modulate
func update_sprite():
	var current_sprite = boots_keys[current_boots_index]	
	if current_sprite == "none":
		boots_sprite.texture = null
		boots_sprite.material = null
	else:
		boots_sprite.texture = Global.boots_collection[current_sprite]
		boots_sprite.material = get_palette_shader(palettes["3color"]["base"], palettes["3color"]["palettes"][singlethreecolours])
	
	Global.selected_boots = current_sprite
	Global.selected_boots_color = boots_sprite.material
...
func _on_collection_button_pressed() -> void:
	current_boots_index = (current_boots_index + 1) % boots_keys.size()
	update_sprite()
...
func _on_random_button_pressed() -> void:
	current_boots_index = rng_sprite.randi_range(1, Global.boots_collection.size() - 1) % boots_keys.size()
