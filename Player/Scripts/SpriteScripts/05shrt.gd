...
func update_sprite():
	...
	if current_sprite == "none":
		...
		shirt_sprite.material = null
	else:
		shirt_sprite.texture = Global.shirt_collection[current_sprite]
		shirt_sprite.material = get_palette_shader(palettes["3color"]["base"], palettes["3color"]["palettes"][singlethreecolours])
	
	Global.selected_shirt = current_sprite
	Global.selected_shirt_color = shirt_sprite.material
...
func _on_random_button_pressed() -> void:
	current_shirt_index = rng_sprite.randi_range(1, Global.shirt_collection.size() - 1) % shirt_keys.size()
