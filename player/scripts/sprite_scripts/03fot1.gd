...
# Node ref
@onready var foot_sprite = $Sprite2D
...
func set_sprite_keys():
	foot_keys = Global.foot_collection.keys()

# Updates texture & modulate
func update_sprite():
	var current_sprite = foot_keys[current_foot_index]
	if current_sprite == "none":
		foot_sprite.texture = null
		foot_sprite.material = null
	else:
		foot_sprite.texture = Global.foot_collection[current_sprite]
		foot_sprite.material = get_palette_shader(palettes["3color"]["base"], palettes["3color"]["palettes"][singlethreecolours])
	
	Global.selected_foot = current_sprite
	Global.selected_foot_color = foot_sprite.material
...
func _on_collection_button_pressed() -> void:
	current_foot_index = (current_foot_index + 1) % foot_keys.size()
	update_sprite()
...
func _on_random_button_pressed() -> void:
	current_foot_index = rng_sprite.randi_range(1, Global.foot_collection.size() - 1) % foot_keys.size()
