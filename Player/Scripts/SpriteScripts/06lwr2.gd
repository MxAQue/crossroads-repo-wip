...
# Node ref
@onready var overalls_sprite = $Sprite2D
...
func set_sprite_keys():
	overalls_keys = Global.overalls_collection.keys()

# Updates texture & modulate
func update_sprite():
	var current_sprite = overalls_keys[current_overalls_index]	
	if current_sprite == "none":
		overalls_sprite.texture = null
		overalls_sprite.material = null
	else:
		overalls_sprite.texture = Global.overalls_collection[current_sprite]
		overalls_sprite.material = get_palette_shader(palettes["3color"]["base"], palettes["3color"]["palettes"][singlethreecolours])
	
	Global.selected_overalls = current_sprite
	Global.selected_overalls_color = overalls_sprite.material
...
func _on_collection_button_pressed() -> void:
	current_overalls_index = (current_overalls_index + 1) % overalls_keys.size()
	update_sprite()
...
func _on_random_button_pressed() -> void:
	current_overalls_index = rng_sprite.randi_range(1, Global.overalls_collection.size() - 1) % overalls_keys.size()
