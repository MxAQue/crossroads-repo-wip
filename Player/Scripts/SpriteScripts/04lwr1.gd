...
# Node ref
@onready var legs_sprite = $Sprite2D
...
func set_sprite_keys():
	legs_keys = Global.legs_collection.keys()

# Updates texture & modulate
func update_sprite():
	var current_sprite = legs_keys[current_legs_index]	
	if current_sprite == "none":
		legs_sprite.texture = null
		legs_sprite.material = null
	else:
		legs_sprite.texture = Global.legs_collection[current_sprite]
		legs_sprite.material = get_palette_shader(palettes["3color"]["base"], palettes["3color"]["palettes"][singlethreecolours])
	
	Global.selected_legs = current_sprite
	Global.selected_legs_color = legs_sprite.material
...
func _on_collection_button_pressed() -> void:
	current_legs_index = (current_legs_index + 1) % legs_keys.size()
	update_sprite()
...
func _on_random_button_pressed() -> void:
	current_legs_index = rng_sprite.randi_range(1, Global.legs_collection.size() - 1) % legs_keys.size()
