...
# Node ref
@onready var neck_sprite = $Sprite2D
...
# Keys
var neck_keys = []
var current_neck_index = 0
var singlefourcolours: int = 0
...
func set_sprite_keys():
	neck_keys = Global.neck_collection.keys()

# Updates texture & modulate
func update_sprite():
	var current_sprite = neck_keys[current_neck_index]	
	if current_sprite == "none":
		neck_sprite.texture = null
		neck_sprite.material = null
	else:
		neck_sprite.texture = Global.neck_collection[current_sprite]
		neck_sprite.material = get_palette_shader(palettes["4color"]["base"], palettes["4color"]["palettes"][singlefourcolours])
	
	Global.selected_neck = current_sprite
	Global.selected_neck_color = neck_sprite.material
...
func _on_collection_button_pressed() -> void:
	current_neck_index = (current_neck_index + 1) % neck_keys.size()
	update_sprite()

func _on_color_button_pressed() -> void:
	singlefourcolours = (singlefourcolours + 1) % palettes["4color"]["palettes"].size()
	update_sprite()

func _on_random_button_pressed() -> void:
	current_neck_index = rng_sprite.randi_range(1, Global.neck_collection.size() - 1) % neck_keys.size()
	singlefourcolours = rng_colors.randi_range(1, palettes["4color"]["palettes"].size() - 1) % palettes["4color"]["palettes"].size()
	update_sprite()
