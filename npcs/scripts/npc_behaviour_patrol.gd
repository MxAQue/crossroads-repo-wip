## npc_behaviour_patrol.gd
@tool

extends NPCBehaviour

const COLOURS = [Color(1,0,0), Color(1,0.5,0), Color(1,1,0), Color(0,1,0), Color(0,0.5,1), Color(0.5,0,1)]
	...
	if Engine.is_editor_hint():
		if patrol_locations.size() > 0:
			for i in patrol_locations.size():
				...
				_p.modulate = _get_colours_by_index(i)
...
# Colours of patrol route nodes
func _get_colours_by_index(i : int) -> Color:
