...
# Node ref
@onready var hands_sprite = $Sprite2D
...
func set_sprite_keys():
	hands_keys = Global.hands_collection.keys()

# Updates texture & modulate
func update_sprite():
	var current_sprite = hands_keys[current_hands_index]	
	if current_sprite == "none":
		hands_sprite.texture = null
		hands_sprite.material = null
	else:
		hands_sprite.texture = Global.hands_collection[current_sprite]
		hands_sprite.material = get_palette_shader(palettes["3color"]["base"], palettes["3color"]["palettes"][singlethreecolours])
	
	Global.selected_hands = current_sprite
	Global.selected_hands_color = hands_sprite.material
...
func _on_collection_button_pressed() -> void:
	current_hands_index = (current_hands_index + 1) % hands_keys.size()
	update_sprite()
...
func _on_random_button_pressed() -> void:
	current_hands_index = rng_sprite.randi_range(1, Global.hands_collection.size() - 1) % hands_keys.size()
