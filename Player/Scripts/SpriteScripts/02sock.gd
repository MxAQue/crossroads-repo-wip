...
# Node ref
@onready var sock_sprite = $Sprite2D
...
func set_sprite_keys():
	sock_keys = Global.sock_collection.keys()

# Updates texture & modulate
func update_sprite():
	var current_sprite = sock_keys[current_sock_index]	
	if current_sprite == "none":
		sock_sprite.texture = null
		sock_sprite.material = null
	else:
		sock_sprite.texture = Global.sock_collection[current_sprite]
		sock_sprite.material = get_palette_shader(palettes["3color"]["base"], palettes["3color"]["palettes"][singlethreecolours])
	
	Global.selected_sock = current_sprite
	Global.selected_sock_color = sock_sprite.material
...
func _on_collection_button_pressed() -> void:
	current_sock_index = (current_sock_index + 1) % sock_keys.size()
	update_sprite()
...
func _on_random_button_pressed() -> void:
	current_sock_index = rng_sprite.randi_range(1, Global.sock_collection.size() - 1) % sock_keys.size()
