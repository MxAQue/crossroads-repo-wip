...
func set_sprite_keys():
	head_keys = Global.head_collection.keys()

# Updates texture & modulate
func update_sprite():
	var current_sprite = head_keys[current_head_index]	
	if current_sprite == "none":
		head_sprite.texture = null
		head_sprite.material = null
	else:
		head_sprite.texture = Global.head_collection[current_sprite]
		head_sprite.material = get_palette_shader(palettes["4color"]["base"], palettes["4color"]["palettes"][singlefourcolours])
	
	Global.selected_head = current_sprite
	Global.selected_head_color = head_sprite.material
...
func _on_collection_button_pressed() -> void:
	current_head_index = (current_head_index + 1) % head_keys.size()
	update_sprite()
...
func _on_random_button_pressed() -> void:
	current_head_index = rng_sprite.randi_range(1, Global.head_collection.size() - 1) % head_keys.size()
